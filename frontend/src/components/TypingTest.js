import React, { Children, isValidElement, cloneElement, useState, useEffect, useContext } from 'react';
import { Box, VStack } from "@chakra-ui/react";
import { useSelector, useDispatch } from "react-redux"
import { putAwayTestText, getTestStateClone, getTypingTest, handleBackspace, timerStarted, resetTypingTest, exportFinishedResults, endOfTypingText } from "../utils/typingtest";
...
export default function TypingTest({children, type, language, initFunc, updateFunc, requestTestTextFunc, ...props}) {
    ...
    useEffect(() => {
        if ((type === "programmer") && (!didTimerStart) && (Object.keys(currentText).length !== 0)) {
            initFunc();
        ...
        <Box>
            <VStack>
                {Children.map( children, (child) => {
                    ...
                    return cloneElement(child, { ...child.props, handleKeyPress: handleKeyPress, handleKeyDown: handleKeyDown, testText: currentText, currentLine: getTypingTest().currentLine, currentIndex: getTypingTest().currentIndex});
                })}
            </VStack>
        </Box>
    );
