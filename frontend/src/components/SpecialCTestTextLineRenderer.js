    ...
    return (
        <Box required="true">
            <Text fontFamily="Iosevka" fontSize={{base:"3xl",lg:"5xl"}} mx="10px">
                {
                props.testTextLine.length === 0 ? <pre style={{"font-family":"Iosevka"}}></pre> :
                    props.testTextLine.map((obj,index) => {
                        if (obj.given === "") {
                            ...
                            return ( <span style={{'color':'#A5ABBD'}}><span css={ ( props.isCurrentLine && index === props.currentIndex ) ? CursorWrapper : css({'color':'#A5ABBD'})}>{obj.expected}</span>{" "}</span>);
